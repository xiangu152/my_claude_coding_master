---
title: "延迟监控"
source: "https://redis.com.cn/topics/latency-monitor.html"
version: "7.x"
---

# 延迟监控

> 原始文档来源：https://redis.com.cn/topics/latency-monitor.html
...
不同监控代码路径有不同的名称，并称之为事件。例如，command{.highlighter-rouge}是测量可能很慢的命令的执行延迟毛刺的事件，而fast-command{.highlighter-rouge}则是监控时间复杂度为O(1)和O(log N)的命令的事件名称。其他事件则不太通用，主要监控Redis执行的特殊操作。例如，fork{.highlighter-rouge}事件仅仅监控Redis执行系统调用fork(2){.highlighter-rouge}所耗的时间。
...
延迟监控子系统的用户界面是LATENCY{.highlighter-rouge}命令。像其他Redis命令一样，LATENCY{.highlighter-rouge}可以接收子命令来改变命令的行为。接下来的章节对各个子命令进行说明。

LATENCY LATEST

LATENCY LATEST{.highlighter-rouge}命令会上报已记录事件的最后一次延迟。每个事件包含以下字段：
...
最大事件延迟所指的所有时间段并不是指从Redis实例启动以来，因为事件数据有可能会用稍后看到的LATENCY RESET{.highlighter-rouge}命令来重置。
...
LATENCY HISTORY event-name{.highlighter-rouge}

LATENCY HISTORY{.highlighter-rouge}命令从时间序列中提取原始数据时是非常有用的，数据是"时间戳-延迟"值对的形式。本命令会返回给定事件的160个元素。应用程序可用提取的原始数据来实现性能监控和展示图表等功能。
...
LATENCY RESET [event-name{.highlighter-rouge} ... event-name{.highlighter-rouge}]

LATENCY RESET{.highlighter-rouge}命令，若不带参数，则重置所有事件，丢弃当前记录的延迟毛刺事件并重置最大事件延迟的值。

通过指定事件名称作为参数，可以重置指定的事件。该命令在执行期间，会返回已被重置的事件时间序列的序号。

LATENCY GRAPH event-name{.highlighter-rouge}
...
graph子命令非常有用，能快速判断指定事件的延迟趋势，不需要使用额外的工具，也不需要解析LATENCY HISTORY{.highlighter-rouge}命令提供的原始数据。

LATENCY DOCTOR

LATENCY DOCTOR{.highlighter-rouge}命令是最强大的延迟监控分析工具，能提供更多统计数据，如延迟毛刺间的平均时间间隔，中值偏差和易懂的事件分析。对某些事件，如"fork"事件，会提供如系统创建进程的速率等更多的信息。
...
LATENCY DOCTOR{.highlighter-rouge}命令目前尚不完善（原意：有精神病行为），因此，我们建议谨慎使用该命令。
