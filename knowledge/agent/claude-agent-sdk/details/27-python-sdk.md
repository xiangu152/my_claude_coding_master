...
> 原始文档来源：https://code.claude.com/docs/en/agent-sdk/python
...
| can_use_tool | CanUseTool | None | None | Tool permission callback, invoked only when the permission flow falls through to a prompt. Not invoked for calls auto-approved by allowed_tools, allow rules, or permission_mode. See CanUseTool for details |
...
| skills | list[str] | Literal["all"] | None | None | Skills available to the session. Pass "all" to enable every discovered skill, or a list of skill names. When set, the SDK adds the Skill tool to allowed_tools automatically. If you also pass tools, include "Skill" in that list. See Skills |
...
The context-1m-2025-08-07 beta is retired as of April 30, 2026. Passing this header with Claude Sonnet 4.5 or Sonnet 4 has no effect, and requests that exceed the standard 200k-token context window return an error. To use a 1M-token context window, migrate to Claude Sonnet 5, Claude Sonnet 4.6, Claude Opus 4.6, Claude Opus 4.7, or Claude Opus 4.8, which include 1M context at standard pricing with no beta header required.
...
If permission_mode is set to bypassPermissions and allow_unsandboxed_commands is enabled, the model can autonomously execute commands outside the sandbox without approval prompts (an explicit ask rule still forces one). This combination effectively allows the model to escape sandbox isolation silently.
