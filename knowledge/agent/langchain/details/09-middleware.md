---
title: "中间件"
source: "https://langchain-doc.cn/v1/python/langchain/middleware.html"
version: "v1"
---

# 中间件

> 原始文档来源：https://langchain-doc.cn/v1/python/langchain/middleware.html

---
中间件

控制并定制代理（Agent）执行的每一个步骤 代理与过滤
...
LangChain 为常见用例提供了预构建的中间件： LangChain教程

摘要 (Summarization)

在接近上下文窗口（token limits）限制时，自动对对话历史进行摘要。
...
在工具执行之前，暂停代理执行以供人工批准、编辑或拒绝工具调用。 软件
...
⚠️ 重要提示： “人在回路中”中间件需要一个检查点（checkpointer）来在中断期间维护状态。
...
ℹ️ 了解更多： 请参阅 Anthropic 提示词缓存策略和限制。
...
检测和处理对话中的个人身份信息（Personally Identifiable Information, PII）。
...
tools: 字符串 | BaseTool 列表，要模拟的工具名称（字符串）或 BaseTool 实例列表。如果为 None（默认），所有工具将被模拟。如果为空列表，则不会模拟任何工具。
...
通过实现运行在代理执行流程中特定点的钩子（hooks）来构建自定义中间件。
