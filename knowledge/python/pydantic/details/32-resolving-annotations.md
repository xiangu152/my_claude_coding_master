...
> 原始文档来源：https://pydantic.dev/docs/validation/latest/internals/resolving_annotations/
...
Fetch the __annotations__ key from the current base class’ __dict__, if present. For Base, this will be {'f1': 'MyType'}.
Iterate over the __annotations__ items and try to evaluate the annotation 1 using a custom wrapper around the built-in eval() function. This function takes two globals and locals arguments:
...
If the annotation failed to evaluate, it is kept as is, so that the model can be rebuilt at a later stage. This will be the case for f5.
...
This is done unconditionally, as forward annotations can be only present as part of a type hint (e.g. Optional['int']), as dictated by the typing specification. ↩
