...
# Dataclasses
> 原始文档来源：https://pydantic.dev/docs/validation/latest/concepts/dataclasses/
...
Configuration support (note that dataclasses doesn’t support the model_config attribute as with Pydantic models)
Nested classes
Arguments used to instantiate the dataclass are also copied.

Some differences between Pydantic dataclasses and models include:

The various methods to validate, dump and generate a JSON Schema aren’t available. Instead, you can wrap the dataclass with a TypeAdapter and make use of its methods:
...
Validators (see the dedicated section).

The extra configuration behavior:

Extra data is not included in serialization.
There is no way to customize validation of extra values using the __pydantic_extra__ attribute.
...
The rebuild_dataclass() function can be used to rebuild the core schema of the dataclass. See the rebuilding model schema section for more details.
