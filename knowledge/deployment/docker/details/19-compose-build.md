...
> 原始文档来源：https://docs.docker.com/reference/compose-file/build/
...
additional_contexts
Requires:
Docker Compose 2.17.0 and later
...
entitlements
Requires:
Docker Compose 2.27.1 and later
...
provenance
Requires:
Docker Compose 2.39.0 and later

provenance configures the builder to add a provenance attestation to the published image.
...
sbom configures the builder to add a provenance attestation to the published image. The value can be either a boolean to enable/disable sbom attestation, or a key=value string to set SBOM generator configuration. This let you select an alternative SBOM generator image (see https://github.com/moby/buildkit/blob/master/docs/attestations/sbom-protocol.md)
...
ulimits
Requires:
Docker Compose 2.23.1 and later
