...
> 原始文档来源：https://docs.docker.com/engine/daemon/
...
Using the http-proxy, https-proxy, and no-proxy fields in the daemon configuration file (Docker Engine version 23.0 or later).
Using the --http-proxy, --https-proxy, and --no-proxy command-line options. (Docker Engine version 23.0 or later).
