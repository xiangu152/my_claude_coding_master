...
> 原始文档来源：https://docs.docker.com/docker-hub/
...
In addition to the graphical interface, you can interact with Docker Hub using the Docker Hub API, the experimental Docker Hub CLI tool, or the Docker Hub MCP server for AI agent integrations.
...
Explore the content library, featuring millions of images for operating systems, frameworks, databases, and more.
...
Learn about settings in Docker Hub.
...
Explore usage limits and how to better utilize Docker Hub.

MCP server

Use the Docker Hub MCP server to access image metadata and manage repositories with AI agents.
...
Optional. Select the plus icon to add and configure more build rules.
...
All of these options are available from the Build configuration screen for each repository. In Docker Hub, select My Hub > Repositories, and select the name of the repository you want to edit. Select the Builds tab, and then select Configure Automated builds.
